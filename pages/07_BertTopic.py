...
if "df_working" not in st.session_state:
    ...
    if 'berttopic_model' not in st.session_state or st.session_state.berttopic_model == False:
        ...
        proceed_button_bert =  st.button('Proceed with Bert topic analysis')
        ...
        if proceed_button_bert:
            ...
            st.session_state.berttopic_model = berttopic_model
    ...
    df_working = pd.concat([df_working,topic_series,probability_series],axis = 1)

    # get topic info
    topic_information = berttopic_model.get_topic_info()

    # number of topics
    st.write('number of topics is', len(topic_information))
    ...
    for topic in df_working['bert_topic_id'].unique():
        #print(topic)
        ...
        temp_df_v2 = df_working[df_working['bert_topic_id'] == topic]
    ...
    proper_cols = list(set(df_working.bert_topic_id.values.tolist()))
    ...
    st.write('Only view by titles',df_most_rep_docs_v2)
    ...
    if st.session_state.useful_document_vis == False:
        with st.spinner('Loading document viewer, reduces embeddings to 2d image'):
            useful_document_vis = berttopic_model.visualize_documents(df_working.Data.values.tolist())
    ...
    with st.spinner('Loading hierarchical tree diagram'):
        hierarchical_topics = berttopic_model.hierarchical_topics(df_working.Data.values.tolist())
    ...
    with st.spinner('Loading topics over time'):
        timestamps = df_working.Year.values.tolist()

        topics_over_time = berttopic_model.topics_over_time(df_working.Data.values.tolist(), timestamps)
